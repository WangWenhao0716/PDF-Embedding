...
__all__ = [
    ...
    'deit_base_distilled_patch16_384', 'deit_base_query_patch16_224'
]

class QueryVisionTransformer(VisionTransformer):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.dist_token = nn.Parameter(torch.zeros(1, 5, self.embed_dim))
        num_patches = self.patch_embed.num_patches
        self.pos_embed = nn.Parameter(torch.zeros(1, num_patches + 6, self.embed_dim))
...
class VisionTransformerPattern(VisionTransformer):
    def __init__(self, *args, **kwargs):
        ...
        trunc_normal_(self.pattern_token, std=.02)
        trunc_normal_(self.pos_embed, std=.02)
...
@register_model
def deit_base_pattern_patch16_224(pretrained=False, **kwargs):
    model = VisionTransformerPattern(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
...
@register_model
def deit_base_query_patch16_224(pretrained=True, **kwargs):
    model = QueryVisionTransformer(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
        ...
        pos_embed_pretrained = checkpoint["model"]['pos_embed']
        repeated_pos = pos_embed_pretrained[:, 0, :].unsqueeze(1).repeat(1, 6, 1)  # shape [1, 6, 768]
        remaining_pos = pos_embed_pretrained[:, 1:, :]  # shape [1, 196, 768]
